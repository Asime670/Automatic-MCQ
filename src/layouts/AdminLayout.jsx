...
import { FiHome, FiFileText, FiPieChart, FiLogOut, FiUsers, FiMenu, FiX } from 'react-icons/fi';
...
export default function AdminLayout() {
  ...
  const menu = [
    ...
    { name: 'Results', path: '/admin/results', icon: FiUsers },
  ];
    ...
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <div className={`fixed inset-y-0 left-0 z-30 w-64 bg-white dark:bg-gray-800 shadow-md flex flex-col transform transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-16 flex items-center justify-between px-6 border-b border-gray-200 dark:border-gray-700">
          <span className="text-xl font-bold text-primary-600 dark:text-primary-400">Admin Portal</span>
        ...
        <nav className="flex-1 px-4 py-4 space-y-2">
          {menu.map((item) => {
            ...
            return (
              <Link key={item.name} to={item.path} className={`flex items-center px-4 py-3 rounded-lg transition-colors ${isActive ? 'bg-primary-50 text-primary-600 dark:bg-primary-900/50 dark:text-primary-400' : 'text-gray-600 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-700'}`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden w-full">
        <header className="h-16 bg-white dark:bg-gray-800 shadow-sm flex items-center px-4 lg:px-8 justify-between">
          ...
          <Link to="/student" className="px-3 py-1.5 bg-primary-100 text-primary-700 hover:bg-primary-200 rounded-md text-sm font-medium transition-colors dark:bg-primary-900/30 dark:text-primary-400">
